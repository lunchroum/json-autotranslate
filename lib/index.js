...
commander_1.default
    ...
    .option("-g, --glossaries <glossariesDir>", "set the glossaries folder to be used by DeepL")
    ...
    .option("-o, --overwrite-output-file", "outputs single translated file for each targetLanguage or overwrites the existing file")
    ...
    .parse(process.argv);
const translate = async (inputDir = ".", cacheDir = ".json-autotranslate-cache", sourceLang = "en", deleteUnusedStrings = false, overwriteOutputFile = false, fileType = "auto", withArrays = false, dirStructure = "default", fixInconsistencies = false, service = "google-translate", matcher = "icu", decodeEscapes = false, config, glossariesDir, appName, context) => {
    ...
    for (const language of targetLanguages) {
        ...
        const translateContent = createTranslator(translationService, service, sourceLang, language, cacheDir, workingDir, dirStructure, deleteUnusedStrings, withArrays, overwriteOutputFile);
        switch (dirStructure) {
            case "default": {
...
translate(commander_1.default.input, commander_1.default.cache, commander_1.default.sourceLanguage, commander_1.default.deleteUnusedStrings, commander_1.default.overwriteOutputFile, commander_1.default.type, commander_1.default.withArrays, commander_1.default.directoryStructure, commander_1.default.fixInconsistencies, commander_1.default.service, commander_1.default.matcher, commander_1.default.decodeEscapes, commander_1.default.config, commander_1.default.glossaries, commander_1.default.appName, commander_1.default.context).catch((e) => {
    console.log();
...
function createTranslator(translationService, service, sourceLang, targetLang, cacheDir, workingDir, dirStructure, deleteUnusedStrings, withArrays, overwriteOutputFile) {
    return async (sourceFile, destinationFile) => {
        ...
        if (service !== "dry-run") {
            ...
            const translatedFile = {
                ...(0, omit_1.default)(existingTranslations, deleteUnusedStrings ? unusedStrings : []),
                ...newKeys,
            };
            ...
            fs.writeFileSync(path.resolve((0, file_system_1.evaluateFilePath)(workingDir, dirStructure, targetLang), outputFileName ?? destinationFile?.name ?? sourceFile.name), newContent);
            ...
            fs.writeFileSync(path.resolve(languageCachePath, outputFileName ?? destinationFile?.name ?? sourceFile.name), `${JSON.stringify(translatedFile, null, 2)}\n`);
        }
